...
func move_to_player(mob: CharacterBody2D):
	...
	var velocity = accelarate_to_direction(direction)
...
func accelarate_to_direction(direction: Vector2):
	var final_velocity = max_speed * direction
	current_velocity = current_velocity.lerp(final_velocity, 1 - exp(-accelaration * get_process_delta_time()))
